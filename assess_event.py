...
def compare_iter(model, dataloader):
    ...
    for input, label in dataloader:
        ...
        with torch.no_grad():
            model(input)
        # for event-based dataset model.feature: (N, T, D)
        feature_map = model.feature.mean(dim=1).cpu().detach().numpy()  # (N, D)
...
def assess_all(model, dataloader):
    ...
    for input, label in dataloader:
        input = input.cuda()
        with torch.no_grad():
            output = model(input)  # (N, T, C)
            output = output.mean(axis=1).softmax(dim=1)  # (N, C)
        ...
        # logME
        feature_map = model.feature.mean(dim=1).cpu().detach().numpy()
        feature_maps.append(feature_map)
        # label
        label = label.cpu().detach().numpy()
        labels.append(label)
