...
def parser_args():
    ...
    parser.add_argument('--pretrained_path', default='/home/haohq/SNN-Trans-Assess-main/weights/sew18_checkpoint_319.pth', type=str, help='path to pre-trained weights')
    ...
    parser.add_argument('--output_dir', default='output/dvs128gesture/', help='path where to save')
    parser.add_argument('--save_freq', default=10, type=int, help='save frequency')
    parser.add_argument('--sched', default='StepLR', type=str, help='scheduler')
    parser.add_argument('--step_size', default=10, type=int, help='step size for scheduler')
    parser.add_argument('--gamma', default=0.2, type=float, help='gamma for scheduler')
    # dist
...
def load_data(args):
    ...
    if args.dataset == 'asl_dvs':  
        ...
        train_dataset = dataset(root=args.root, train=True, data_type='frame', frames_number=args.nsteps, split_by='time', duration=args.duration)
        test_dataset = dataset(root=args.root, train=False, data_type='frame', frames_number=args.nsteps, split_by='time', duration=args.duration)
...
def train(
    ...
    while(epoch < nepochs):
        ...
        if epoch % args.save_freq == 0:
            ...
            save_name = 'checkpoint/epoch{}.pth'.format(epoch, top1_accuracy)
